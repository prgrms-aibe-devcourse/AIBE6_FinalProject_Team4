 ...
 *
 * <p><b>캐시 키는 정규화된 종 이름·응답 버전·원본 컨텍스트 fingerprint다.</b> 사용자가 자유롭게 입력하는 종 이름을 그대로 키로 쓴다.
 */
@Getter
@Entity
@Table(
...
public class PlantCareGuideCache extends BaseEntity {
  ...
  /**
   * 생성 시점에 사용한 검증 재배 근거 fingerprint. 원본 변경 시 기존 캐시를 무효화한다.
   *
   * <p><b>NOT NULL이어야 한다.</b> MySQL 유니크 인덱스는 NULL끼리를 서로 다른 값으로 취급해서, 이 컬럼이 비면 위 유니크 제약이 같은 종·같은 버전의
   * 중복 행을 전혀 막지 못한다. 근거가 없는 종도 빈 문자열을 해싱해 값이 항상 채워진다.
   */
