 ...
 * 예산을 통째로 소진할 수 있었다. 그러면 남용한 본인이 아니라 <b>다른 모든 사용자의 AI 기능이 그날 내내 멈춘다</b>. 이 창은 그 사이를 메워, 한 계정의 손해가
 * 그 계정 안에서 끝나게 한다.
 *
 * <p><b>기능을 나누지 않고 합산한다.</b> {@link AiRateLimitWindow}와 달리 feature 컬럼이 없다. 기능별로 따로 세면 사용자가 기능을
 * 번갈아 호출해 한도를 기능 수만큼 곱해 쓸 수 있고, 그러면 전역 예산을 지킨다는 목적이 그대로 무너진다. 전역 예산이 기능을 구분하지 않으므로 이 창도 구분하지 않는다.
 *
