...
public class PlantGrowthContextService implements PlantGrowthContextQuery {
	...
	@Override
	public PlantGrowthContextResponse getGrowthContext(Long userId, Long profileId, int recentJournalLimit) {
		validateIds(userId, profileId);
		if (recentJournalLimit < 1 || recentJournalLimit > MAX_RECENT_JOURNAL_LIMIT) {
			throw new BusinessException(ErrorCode.COMMON_VALIDATION_FAILED, "최근 일지 조회 개수가 올바르지 않습니다.");
		}
		...
		List<PlantJournal> recentJournals = plantJournalRepository.findRecentActiveByProfile(
				userId, profileId, PageRequest.of(0, recentJournalLimit));
