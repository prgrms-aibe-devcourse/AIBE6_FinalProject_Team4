...
public class KakaoOAuthClient implements OAuthClient {
	...
	public OAuthUserInfo fetchUserInfo(String code, String state) {
		try {
			...
			form.add("code", code);
			// Kakao 앱에서 "Client Secret" 사용을 켠 경우에만 필요 — 꺼져 있는데 보내면 오히려 오류가 난다.
			...
			if (userInfo == null || userInfo.kakaoAccount() == null || userInfo.kakaoAccount().email() == null) {
				throw new BusinessException(ErrorCode.AUTH_OAUTH_EMAIL_REQUIRED);
			}

			String nickname = userInfo.kakaoAccount().profile() != null
					? userInfo.kakaoAccount().profile().nickname()
					: null;
			return new OAuthUserInfo(String.valueOf(userInfo.id()), userInfo.kakaoAccount().email(), nickname);
		} catch (RestClientResponseException e) {
			log.warn("Kakao OAuth 실패: status={}, body={}", e.getStatusCode(), e.getResponseBodyAsString());
			throw new BusinessException(ErrorCode.AUTH_OAUTH_AUTHENTICATION_FAILED);
		} catch (RestClientException e) {
			log.warn("Kakao OAuth 실패", e);
			throw new BusinessException(ErrorCode.AUTH_OAUTH_AUTHENTICATION_FAILED);
		}
	}
