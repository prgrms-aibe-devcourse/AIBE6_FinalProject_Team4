...
public class GachaDrawEngine {
  ...
  static final int QA_HYPER_BOOST_START = 1_260_000;
  static final int QA_HYPER_BOOST_END = 1_470_000;
  ...
  public RolledGrade rollGrade(boolean qaHyperRateBoostEnabled) {
    int rollValue = randomSource.nextInt(TOTAL_WEIGHT);
    return new RolledGrade(rollValue, rarityFor(rollValue, qaHyperRateBoostEnabled));
  }
  ...
  public TradingCardRarity rarityFor(int rollValue, boolean qaHyperRateBoostEnabled) {
    if (rollValue < 0 || rollValue >= TOTAL_WEIGHT) {
      throw new IllegalArgumentException("rollValue must be between 0 and 2099999.");
    }
    if (qaHyperRateBoostEnabled
        && rollValue >= QA_HYPER_BOOST_START
        && rollValue < QA_HYPER_BOOST_END) {
