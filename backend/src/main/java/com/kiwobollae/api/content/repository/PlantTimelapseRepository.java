...
public interface PlantTimelapseRepository extends JpaRepository<PlantTimelapse, Long> {
	...
	// PROCESSING -> FAILED 조건부 전이(복구 스케줄러 전용). 0을 반환하면 그 사이 정상 워커가 이미
	// 완료/실패 처리했다는 뜻이라 알림을 다시 보내지 않는다.
	@Modifying
	@Query("update PlantTimelapse t set t.status = com.kiwobollae.api.content.entity.enums.PlantTimelapseStatus.FAILED, "
			...
			+ "where t.plantProfile.id = :profileId "
			+ "and t.status = com.kiwobollae.api.content.entity.enums.PlantTimelapseStatus.PROCESSING")
	int failIfStillProcessing(@Param("profileId") Long profileId, @Param("reason") String reason,
			@Param("completedAt") LocalDateTime completedAt);

	// 지정 시각 이전에 요청됐는데 아직도 PROCESSING인 행 — 워커가 죽었거나(fail() 자체 실패,
	// 서버 재시작 등) 응답 없이 멈춘 것으로 간주해 복구 스케줄러가 정리 대상으로 삼는다.
	@Query("select t.plantProfile.id from PlantTimelapse t "
			+ "where t.status = com.kiwobollae.api.content.entity.enums.PlantTimelapseStatus.PROCESSING "
			+ "and t.requestedAt < :staleBefore "
			+ "order by t.requestedAt asc")
	List<Long> findStaleProcessingProfileIds(@Param("staleBefore") LocalDateTime staleBefore, Pageable pageable);
