...
public interface JournalCompletionLogRepository extends JpaRepository<JournalCompletionLog, Long> {
	...
	@Query("select (count(c) > 0) from JournalCompletionLog c where c.plantProfile.id = :profileId")
	boolean existsByProfileId(@Param("profileId") Long profileId);

	@Query("select c from JournalCompletionLog c where c.plantJournal.id = :journalId")
	Optional<JournalCompletionLog> findByJournalId(@Param("journalId") Long journalId);
}
