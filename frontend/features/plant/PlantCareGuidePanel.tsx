...
const ENVIRONMENT_ROWS: {
  key: keyof PlantCareGuideData["environment"];
  label: string;
  icon: string;
}[] = [
...
const ERROR_HINTS: Record<string, string> = {
  ...
  AI_PROVIDER_UNAVAILABLE:
    "AI 서비스가 일시적으로 불안정해요. 잠시 뒤에 다시 시도해 주세요.",
  AI_CONFIGURATION_INVALID:
    "지금은 가이드를 제공할 수 없어요. 문제가 계속되면 문의해 주세요.",
...
function toGuideError(requestError: unknown): GuideError {
  if (requestError instanceof ApiError) {
    return {
      ...
      hint: ERROR_HINTS[requestError.code] || "",
    };
  }
  return {
    message: "재배 가이드를 불러오지 못했어요.",
...
export default function PlantCareGuidePanel({
  ...
  const requestGuide = () => {
    ...
    getPlantCareGuide(speciesName, accessToken, controller.signal)
      .then((data) => {
      ...
      {!guide && (
        <>
          ...
          {error && !loading && (
            <div
              role="alert"
              className="mt-3.5 rounded-[11px] bg-danger-soft px-[13px] py-[11px] text-[13px] text-danger"
            >
              ...
              {error.hint && (
                <div className="mt-1.5 pl-[26px] font-medium">{error.hint}</div>
              )}
            </div>
          )}
          ...
          <div className="mt-3.5">
            ...
            {!loading && !error && (
              <div className="mt-2 text-xs text-faint">
                처음 보는 종은 가이드를 새로 만드느라 10초 이상 걸릴 수 있어요.
              </div>
            )}
          </div>
        </>
      ...
      {guide && expanded && (
        <div className="mt-4 flex flex-col gap-5">
          ...
          <div>
            ...
            <p className="mt-2 text-[14.5px] leading-[1.65] text-[#4a5647]">
              {guide.difficultyReason}
            </p>
          </div>

          <div>
            <SubHeading className="mb-2 text-[15px] font-extrabold">
              환경 조건
            </SubHeading>
          ...
          <div>
            ...
            <ul className="flex flex-col gap-2">
              {guide.pitfalls.map((pitfall) => (
                <li
                  key={pitfall.problem}
                  className="rounded-xl bg-[#fdf7ec] px-3.5 py-3"
                >
                  <div className="text-[14px] font-extrabold text-[#8a6d00]">
                    {pitfall.problem}
                  </div>
                  <div className="mt-1 text-[14px] leading-[1.6] text-[#4a5647]">
                    {pitfall.action}
          ...
          <div>
            ...
            <p className="text-[14.5px] leading-[1.65] text-[#4a5647]">
              {guide.harvestTarget}
            </p>
          </div>

          <p className="border-t border-line pt-3 text-xs leading-[1.6] text-faint">
            {DISCLAIMER}
          </p>
        </div>
      )}
