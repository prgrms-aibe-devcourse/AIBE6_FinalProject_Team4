...
const GUIDE: PlantCareGuideData = {
  ...
  pitfalls: [
    ...
    { problem: '열매가 갈라져요', action: '물 주는 간격을 일정하게 유지해 주세요.' },
  ],
...
function renderPanel() {
  return render(
    <PlantCareGuidePanel speciesId={7} speciesName="방울토마토" accessToken="access-token" />,
  );
}
...
describe('PlantCareGuidePanel', () => {
  ...
  it('버튼을 누르기 전에는 가이드를 요청하지 않는다', () => {
    ...
    expect(screen.getByRole('button', { name: '재배 가이드 보기' })).toBeInTheDocument();
  });
  ...
  it('요청한 가이드를 난이도·환경·단계·실패 대처·수확 목표로 나눠 보여준다', async () => {
    ...
    expect(mockedGetGuide).toHaveBeenCalledWith(7, 'access-token', expect.any(AbortSignal));
    expect(await screen.findByText('난이도 초급')).toBeInTheDocument();
    expect(screen.getByText(GUIDE.difficultyReason)).toBeInTheDocument();

    expect(screen.getByRole('heading', { name: '환경 조건' })).toBeInTheDocument();
  ...
  it('가이드와 함께 AI 책임 문구를 노출한다', async () => {
    ...
    expect(
      await screen.findByText('AI가 생성한 참고 정보이며 정확성을 보장하지 않습니다.'),
    ).toBeInTheDocument();
  });
  ...
  it('생성이 끝날 때까지 로딩 상태를 보여준다', async () => {
    ...
    expect(screen.getByRole('status')).toHaveTextContent('가이드를 만들고 있어요');
    expect(screen.getByRole('button', { name: '가이드를 만드는 중...' })).toBeDisabled();
  ...
  it('생성 중 충돌(409) 후 다시 시도하면 가이드를 다시 요청한다', async () => {
    mockedGetGuide
      .mockRejectedValueOnce(
        new ApiError('COMMON_DATA_CONFLICT', '재배 가이드를 생성하고 있습니다. 잠시 후 다시 시도해 주세요.', 409),
      )
  ...
  it('네트워크 오류처럼 서버 메시지가 없으면 기본 안내를 보여준다', async () => {
    ...
    expect(alert).toHaveTextContent('네트워크 상태를 확인하고 다시 시도해 주세요.');
  });
  ...
  it('요청 중에 종이 바뀌면 진행 중이던 요청을 끊는다', () => {
    ...
    rerender(<PlantCareGuidePanel speciesId={8} speciesName="상추" accessToken="access-token" />);
  ...
  it('종이 바뀌면 이전 종의 가이드를 비운다', async () => {
    ...
    await waitFor(() => expect(screen.queryByText('난이도 초급')).not.toBeInTheDocument());
    expect(screen.getByRole('button', { name: '재배 가이드 보기' })).toBeInTheDocument();
  });
  ...
  it('로그인 토큰이 없으면 요청 버튼을 막는다', () => {
    render(<PlantCareGuidePanel speciesId={7} speciesName="방울토마토" accessToken={null} />);

    expect(screen.getByRole('button', { name: '재배 가이드 보기' })).toBeDisabled();
  });
});
