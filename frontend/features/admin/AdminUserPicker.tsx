  ...
  return (
    <section className="overflow-hidden rounded-[18px] bg-white shadow-card">
      <div className="border-b border-line p-5">
        ...
        <p className="mt-1 text-sm text-sub">회원 ID·이메일·닉네임·이름으로 검색할 수 있습니다.</p>
        <div className="mt-4 grid gap-2 sm:grid-cols-[minmax(0,1fr)_150px]">
          ...
          <label htmlFor="admin-user-status" className="sr-only">회원 상태</label>
          <select
      ...
      <div className="overflow-x-auto">
        <table className="w-full min-w-[720px] text-left text-sm">
          ...
          <tbody>
            {loading ? (
              <tr><td colSpan={5} className="px-5 py-10 text-center text-sub">회원을 불러오고 있어요.</td></tr>
            ) : errorMessage ? (
              <tr><td colSpan={5} role="alert" className="px-5 py-10 text-center text-danger">{errorMessage}</td></tr>
            ) : users.length === 0 ? (
              <tr><td colSpan={5} className="px-5 py-10 text-center text-sub">조건에 맞는 회원이 없습니다.</td></tr>
            ) : users.map((user) => {
              const selected = selectedUserId === user.id;
              return (
                <tr key={user.id} className={`border-t border-[#f2f3ec] ${selected ? 'bg-brand-soft/60' : ''}`}>
                  <td className="px-5 py-3.5">
                    <div className="font-bold">{user.nickname}</div>
                    <div className="mt-0.5 text-xs text-sub">{user.name} · #{user.id}</div>
                  </td>
