  ...
  if (view === 'new') {
    return (
      <div className="container">
        <button type="button" onClick={() => setView('list')} className="cursor-pointer rounded-[10px] border-[1.5px] border-line bg-white px-3 py-2 text-sm font-semibold text-sub hover:bg-brand-soft hover:text-brand-dark">← 1:1 문의</button>
    ...
    <div className="container">
      ...
      {section === 'inquiry' ? (
        <>
          ...
          {loading ? (
            <div className="rounded-[22px] bg-white py-14 text-center text-[15px] text-sub shadow-card">문의 내역을 불러오고 있어요</div>
          ) : error ? (
            <div className="rounded-[22px] bg-white px-5 py-14 text-center text-[15px] text-sub shadow-card">{error}</div>
          ) : inquiries.length === 0 ? (
            <div className="rounded-[22px] bg-white py-14 text-center text-[15px] text-sub shadow-card">아직 등록한 문의가 없어요.</div>
          ) : (
                ...
                return (
                  <div key={q.id} id={`inquiry-${q.id}`} className="rounded-2xl bg-white px-5 py-[18px] shadow-[0_2px_10px_rgba(46,54,42,.07)]">
                    ...
                    >
                      ...
                      <span className="material-symbols-outlined text-lg text-faint">
                        {expanded ? 'expand_less' : 'expand_more'}
                      </span>
                    </button>
                    {expanded && (
                      <div className="mt-3.5 whitespace-pre-wrap rounded-xl bg-[#f6f7f1] p-3.5 text-sm leading-[1.6] text-[#4a5647]">
                        {q.content}
                      </div>
                    )}
                    {expanded && q.status === 'ANSWERED' && (
                      <div className="mt-3 rounded-xl border-[1.5px] border-[#dcebc7] bg-[#F6F9EF] p-3.5">
                        <div className="mb-1.5 flex items-center gap-2">
                          <span className="flex h-7 w-7 items-center justify-center rounded-full bg-white shadow-[0_1px_4px_rgba(46,54,42,.15)]">
                            <img src={SEEDLING_ICON_SRC} alt="" className="h-5 w-5" />
                          </span>
                          <div>
                            <div className="text-xs font-extrabold">{q.answerAdminName ?? '키워볼래 지기'}</div>
                            <div className="text-[11px] text-sub">{q.answeredAt ? formatDateTime(q.answeredAt) : ''}</div>
                          </div>
                        </div>
                        <p className="whitespace-pre-wrap text-sm leading-[1.6] text-[#4a5647]">{q.answerContent}</p>
                      </div>
                    )}
                    {expanded && q.status !== 'ANSWERED' && (
                      <div className="mt-3 rounded-xl bg-[#f6f7f1] p-3.5 text-sm text-sub">
                        아직 답변을 준비하고 있어요. 조금만 기다려 주세요.
                      </div>
                    )}
                  </div>
                );
        ...
        <>
          ...
          {reportsLoading ? (
            <div className="rounded-[22px] bg-white py-14 text-center text-[15px] text-sub shadow-card">신고 내역을 불러오고 있어요</div>
          ) : reportsError ? (
            <div className="rounded-[22px] bg-white px-5 py-14 text-center text-[15px] text-sub shadow-card">{reportsError}</div>
          ) : reports.length === 0 ? (
            <div className="rounded-[22px] bg-white py-14 text-center text-[15px] text-sub shadow-card">아직 접수한 신고가 없어요.</div>
          ) : (
            <div className="flex flex-col gap-3">
              {reports.map((r) => {
                ...
                return (
                  <div key={r.id} className="rounded-2xl bg-white px-5 py-[18px] shadow-[0_2px_10px_rgba(46,54,42,.07)]">
                    <button
                      type="button"
